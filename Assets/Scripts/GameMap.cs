...
public class GameMap : MonoBehaviour
{
    ...
    private async void Awake()
    {
        try
        {
            var program = Resources.LoadAsync<TextAsset>("programs/ship");
            await program;
            if (program.asset != null)
            {
                _program = ((TextAsset)program.asset).text;
            }
        }
        catch (Exception e)
        {
    ...
    {
        OnDestroy();  // Clear arrays and ensure contents are disposed of
